...
if __name__ == "__main__":
    ...
    # 使用相对路径构建输入和输出目录
    input_dir = os.path.join(project_root, "processed", "csv")
    output_dir = os.path.join(project_root, "processed", "features")
    
    # 确保输出目录存在
    os.makedirs(output_dir, exist_ok=True)
    
    print(f"从 {input_dir} 读取特征文件")
    print(f"处理后的特征将保存到 {output_dir}")
    
    # 调用 getTCNVideoFeature 函数处理视频特征
    getTCNVideoFeature(input_dir, output_dir)
